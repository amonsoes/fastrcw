    ...
    def report(self, x_hat, y, paths):
        if self.data.adversarial_opt.adversarial:
            if self.data.adversarial_opt.attack_compression:
                ...
                mad_scores = self.data.transforms.attack.mad_score
                ssim_scores = self.data.transforms.attack.ssim_score
                dists_scores = self.data.transforms.attack.dists_score
                l2_norms = self.data.transforms.attack.l2_norm
                self.data.transforms.attack.l2_norm = []
                self.data.transforms.attack.ssim_score = []
                self.data.transforms.attack.mad_score = []
                self.data.transforms.attack.dists_score = []
                self.utils.logger.write_to_report(paths, x_hat, y, mad_scores, ssim_scores, dists_scores, l2_norms)
            else:
                mad_scores = self.data.transforms.attack.mad_score
                ssim_scores = self.data.transforms.attack.ssim_score
                dists_scores = self.data.transforms.attack.dists_score
    ...
    def test_model(self):
        with torch.no_grad():
            ...
            if self.data.transforms.adversarial_opt.adversarial:
                self.data.transforms.attack.set_report_dir(self.utils.logger.run_name)
            ...
            if hasattr(self.data.transforms, 'attack'):
                self.utils.logger.log_iqa(self.data.transforms.attack)
