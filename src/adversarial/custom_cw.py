...
class CW(Attack):
    ...
    def forward_regular(self, images, labels):
        ...
        # sample points in neighborhopod of img and perform CW over all of those to find the superior one
        sample_list = self.get_rand_points_around_sample([images])
        
        best_adv_images_from_starts = None
        best_loss = 9e+07
        self.n = 0 # reset runtime counter
        self.ca_runtime = 0 # reset avg runtime
        ...
        for e, images in enumerate(sample_list):
            
            print(f'\n calculating for random start: {e+1}')
            ...
            for step in range(self.steps):
                ...
                f_loss, outputs = self.get_adversarial_loss(adv_images, labels, target_labels)

                cost = iq_loss + self.c*f_loss
                ...
                self.update_ca_runtime(end_time_step - start_time_step)
                
            
            if cost.item() < best_loss:
                best_adv_images_from_starts = best_adv_images
                best_loss = cost.item()
        if self.protocol_file:
    ...
    def forward_verbose(self, images, labels):
        ...
        for e, images in enumerate(sample_list):
            ...
            for step in range(self.steps):
                ...
                print(f'\ncurrent iteration: {step}, current L2: {current_iq_loss.item()}, current f_loss: {f_loss.item()} current cost: {cost.item()}')
                end_time_step = time.time()
                self.update_ca_runtime(end_time_step - start_time_step)
