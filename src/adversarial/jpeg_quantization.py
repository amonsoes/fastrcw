...
from src.adversarial.jpeg_ifgm import c_quantize_no_rounding, y_quantize_no_rounding, y_quantize, c_quantize
...
class QuantizedJPEGConversion(compress_jpeg):
    
    def __init__(self, device, rounding=torch.round, factor=1):
        super().__init__( device, rounding, factor)
        self.y_quantize = y_quantize(self.device, factor=self.jpeg_quality)
        self.c_quantize = c_quantize(self.device, factor=self.jpeg_quality)
